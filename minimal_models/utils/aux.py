...
def get_file_dir(N, D, d, C, alpha, max_dist=None, mode=None, seed=None, rel_pos_bias=None, optimizer=None, suffix=None):
    base_path = os.path.join(data_dir, f"N_{N}_D_{D}_d_{d}_C_{C}_alpha_{alpha}_max_distance_{max_dist}")
    base_path = os.path.join(base_path, f"{mode}_RelPosBias_{rel_pos_bias}_{optimizer}{suffix}")
    return os.path.join(base_path, f"trial_{seed}")

def get_checkpoint_dir(N, D, d, C, alpha, max_dist=None, mode = "mlp", seed = 1, rel_pos_bias=None, optimizer=None, suffix=None):
    file_dir = get_file_dir(N, D, d, C, alpha, max_dist=max_dist, mode=mode, seed=seed, rel_pos_bias=rel_pos_bias, optimizer=optimizer, suffix=suffix)
    file_dir = os.path.abspath(os.path.join(file_dir, "checkpoints"))
    return file_dir
...
def get_params(N, D, d, C, alpha, max_dist,mode = "mlp", seed = 1, iter = 0, rel_pos_bias=None, optimizer=None, suffix=None):
    checkpoint_dir = get_checkpoint_dir(N, D, d, C, alpha, max_dist=max_dist, mode=mode, seed=seed, rel_pos_bias=rel_pos_bias, optimizer=optimizer, suffix=suffix)
