...
class RelativePositionBiasMini(nn.Module):
    ...
    def setup(self):
        ...
        #self.rel_pos_bias = self.param("rel_pos_bias", 
                                      ...
                                      # (2))
        self.rel_pos_bias = self.param("rel_pos_bias", lambda key, shape: jnp.array([0.0, 0.0]), (2,))
    def __call__(self, seq_len: int):
        """
        Compute the relative positional bias matrix for a given sequence length.
        Returns: (1, seq_len, seq_len)
...
class Attention1st(nn.Module):
    ...
    def setup(self):
        self.attn = nn.Dense(2*self.d + self.D, kernel_init=weights_init(scale=1/jnp.sqrt(2*self.D)), use_bias=False)
    ...
    def __call__(self, x):
        ...
        attn_output = jnp.einsum("bij,bjd->bid", attn_weights, v)
...
class Attention2nd(nn.Module):
    ...
    def setup(self):
        self.attn = nn.Dense(2*self.d + self.D, kernel_init=weights_init(scale=1/jnp.sqrt(2*self.D)), use_bias=False)
        self.rel_pos_bias = self.param("rel_pos_bias", nn.initializers.zeros, (self.seq_len,))
    ...
    def __call__(self, x):
        ...
        attn_weights = nn.softmax(attn_weights, axis=-1)
        attn_output = jnp.einsum("bj,bjd->bd", attn_weights, v)
