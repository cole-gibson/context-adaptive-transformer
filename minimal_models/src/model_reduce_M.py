...
class RelativePositionBias(nn.Module):
    ...
    def setup(self):
        ...
        if self.init_method == "L":
            initial_values = jnp.array([0, 0] + [0.0] * (self.max_distance - 2))  # Set the first element to 0.05
...
class Attention1st(nn.Module):
    ...
    def setup(self):
        self.attn = nn.Dense(self.D, kernel_init=weights_init(scale=0.0), use_bias=False)
    ...
    def __call__(self, x):
        seq_len = x.shape[1]
        Mx = self.attn(x).reshape(x.shape[0], seq_len, self.D)
        attn_weights = jnp.einsum("bid,bjd->bij", x, Mx)
...
def diagonal_initializer(key, shape, dtype=jnp.float32):
    D = shape[0]//2  # Assuming square matrix for simplicity

    # Identity block I
    I = jnp.eye(D, dtype=dtype)*0.0001
    A = jnp.zeros((D, D), dtype=dtype)  # Zero block A
    # Construct block rows
    top_row = jnp.concatenate([A, I], axis=1)
    bottom_row = jnp.concatenate([A, A], axis=1)

    # Concatenate vertically
    full_matrix = jnp.concatenate([top_row, bottom_row], axis=0)
    return full_matrix
...
class Attention2nd(nn.Module):
    ...
    seq_len: int
    def setup(self):
        #self.attn = nn.Dense(self.D, kernel_init=diagonal_initializer, use_bias=False)
        self.attn = nn.Dense(self.D, kernel_init=weights_init(scale=0.0), use_bias=False)
        self.rel_pos_bias = self.param("rel_pos_bias", 
    ...
    def __call__(self, x):
        Mx = self.attn(x).reshape(x.shape[0], self.seq_len, self.D)
        attn_weights = jnp.einsum("bd,bjd->bj", x[:, -1], Mx) + self.rel_pos_bias
        attn_weights = nn.softmax(attn_weights, axis=-1)
